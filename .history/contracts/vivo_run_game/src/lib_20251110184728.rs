...
const PLAYER_STATS: Symbol = symbol_short!("PLAYER_STATS");
const QUESTS: Symbol = symbol_short!("QUESTS");
const QUEST_PROGRESS: Symbol = symbol_short!("QUEST_PROG");
...
impl VivoRunGame {
    ...
    pub fn submit_game_score(env: Env, player: Address, score: u64) {
        ...
        if let Some(token) = env.storage().instance().get::<_, Address>(&TOKEN) {
